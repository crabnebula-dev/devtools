...
- `crates/v3/crates/devtools` The Tauri plugin for the upcoming Tauri v3.x versions (currently alpha). `examples/tauri-v3` is a standalone example app for it, supporting both the wry and CEF runtimes.
...
```bash
cargo update && cargo update --manifest-path examples/tauri-v1/Cargo.toml && cargo update --manifest-path crates/v1/Cargo.toml && cargo update --manifest-path crates/v3/Cargo.toml && cargo update --manifest-path examples/tauri-v3/src-tauri/Cargo.toml
```
