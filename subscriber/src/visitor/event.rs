use inspector_protocol_primitives::FieldT;
use std::fmt;
...
impl<F: FieldT> Visit for EventVisitor<F> {
	...
	fn record_str(&mut self, field: &Field, value: &str) {
		match field.name() {
			...
			key => self.fields.push(F::new(key, value.into())),
		}
	}
	...
	fn record_f64(&mut self, field: &Field, value: f64) {
		self.fields.push(F::new(field.name(), value.into()))
	}
