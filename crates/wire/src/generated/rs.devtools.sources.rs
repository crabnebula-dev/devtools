...
pub mod sources_server {
    ...
    #[async_trait]
    pub trait Sources: std::marker::Send + std::marker::Sync + 'static {
    ...
    impl<T> Clone for SourcesServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "rs.devtools.sources.Sources";
    impl<T> tonic::server::NamedService for SourcesServer<T> {
