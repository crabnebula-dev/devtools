...
use tauri_devtools_shared::{Filter, Filterable, Metrics, LogEntry, SpanEntry, SubscriptionParams};
...
impl<R: Runtime> Server<R> {
    ...
    pub fn new(logs: broadcast::Sender<Vec<LogEntry>>,
               spans: broadcast::Sender<Vec<SpanEntry>>, app_handle: AppHandle<R>, metrics: Arc<Mutex<Metrics>>) -> Self {
     Self { logs, spans, app_handle, metrics }
    }
